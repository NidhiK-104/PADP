...
int main(int argc, char *argv[])
{
	...
	//#pragma omp parallel for private(y,color,red,blue,green) schedule(static,10)
	#pragma omp parallel for private(y,color,red,blue,green) schedule(guided,50)
	//#pragma omp parallel for private(y,color,red,blue,green) schedule(dynamic,50) 
	for(x=0;x<w;x++)
		for(y=0;y<h;y++)
		{
